...
include("./db.php");
  ...
  $sql4="SELECT * FROM `TBL_Kategorie_Location` ORDER BY `L_K_ID` ASC";
	...
	//if (($result= $dblink->query($sql)) AND ($result2= $dblink->query($sql2)) AND ($result4= $dblink->query($sql4))) {
    //while (($row=$result->fetch_row()) AND ($row2=$result2->fetch_row()) AND ($row4=$result4->fetch_row())) {
    ...
    while (($row=$result->fetch_row())) {
	  ...
	  <td><a href=\"loc_del.php?lid=$row[0]\">Stand löschen</a></td>
	  <td><a href=\"loc_upd.php?lid=$row[0]\">Stand ändern</a></td>
      </tr>\n";
    }
...
?>
	...
	<button onclick="window.location.href='./loc_add.php'">Stand hinzufügen</button>
